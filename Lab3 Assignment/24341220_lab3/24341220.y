...
parameter_list : parameter_list COMMA type_specifier ID
		{
			...
			for(int i = 0; i < param_name_list.size(); i++) {
				if(param_name_list[i] == $4->getname()) {
					errorlog << "At line no: " << lines << " Multiple declaration of variable " << $4->getname() << " in parameter of " << func_name << endl << endl;
					error_count++;
					break;
				}
    		}
			...
			param_type_list.push_back($3->getname());
    		param_name_list.push_back($4->getname());
		}
		| parameter_list COMMA type_specifier
		{
 		...
 		| type_specifier ID
 		{
            ...
            // store the necessary information about the function parameters
            // They will be needed when you want to enter the function into the symbol table
			param_type_list.push_back($1->getname());
    		param_name_list.push_back($2->getname());
		}
		| type_specifier
		{
...
declaration_list : declaration_list COMMA ID
		  {
			...
			var_name_list.push_back($3->getname());
 		  ...
 		  | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD //array after some declaration
 		  {
            ...
            // you may need to store the variable names to insert them in symbol table here or later
			var_name_list.push_back($3->getname());
			var_size_list.push_back(stoi($5->getname()));
 		  ...
 		  |ID
 		  {
			...
			var_name_list.push_back($1->getname());
			var_size_list.push_back(-1);  
 		  ...
 		  | ID LTHIRD CONST_INT RTHIRD //array
 		  {
            ...
            // you may need to store the variable names to insert them in symbol table here or later
			var_name_list.push_back($1->getname());
			var_size_list.push_back(stoi($3->getname()));  
...
variable : ID 	
			{
				...
				if (found == NULL) {
					...
					$1->set_var_type(found->get_var_type());
				}
					
				$$ = new symbol_info($1->getname(),"varbl");
				if (found) $$->set_var_type(found->get_var_type());
			}
...
expression : logic_expression
	   ...
	   | variable ASSIGNOP logic_expression 	
	   {
			...
			string lhs_type = $1->get_var_type();
            string rhs_type = $3->get_var_type();
...
term :	unary_expression //term can be void because of un_expr->factor
     ...
     |  term MULOP unary_expression
     {
			...
			string op = $2->getname();
			string left_type = $1->get_var_type();
			string right_type = $3->get_var_type();
...
factor	: variable
	...
	| ID LPAREN argument_list RPAREN
	{
		...
		if (func == NULL) {
			...
			errorlog << "At line no: " << lines << " '" << $1->getname() << "' is not a function" << endl << endl;
			error_count++;
		} else {
			$$->set_var_type(func->get_return_type());
...
arguments : arguments COMMA logic_expression
		  {
				...
				arg_type_list.push_back($3->get_var_type());
		  }
	      | logic_expression
	      {
				...
				arg_type_list.clear();
				arg_type_list.push_back($1->get_var_type());
		  }
	      ;
